...
LEFT JOIN 
	...
	WHERE 
		sub_table.category_code IN (111301,114201) 
		AND `year` =2018
		AND quarter =4
	) table_18 
ON
...
WHERE 
	table_08 .category_code IN (111301,114201) 
...
ON
	main.`year`+1 =secondary.`year`  -- opět překlopení tabulky na sebe a posunutí hodnot o jeden rok 
	AND main.quarter =secondary.quarter 
	AND main.category_code =secondary.category_code
	AND main.industry_branch_code =secondary.industry_branch_code
WHERE 
	secondary.year IS NOT NULL
...
WHERE
	secondary.payroll_year IS NOT NULL
GROUP BY 
	main.payroll_year,
	main.industry_branch_code  
ORDER BY 
